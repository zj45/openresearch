...
**Q: 什么时候用 GraphRAG，什么时候用普通的 atom 查询？**

A: 需要"查找相关内容"时用 GraphRAG，需要"看某个具体 atom"时用普通查询。如果你不确定，直接向 Agent 描述需求，它会自动选择。

**Q: 语义搜索找不到想要的结果怎么办？**

A: 尝试：(1) 换一种方式描述你的查询；(2) 降低 `semanticThreshold`；(3) 增大 `maxAtoms`；(4) 去掉类型过滤。

**Q: 社区检测结果不理想怎么办？**

A: 社区检测基于图的连接结构，如果 atoms 之间缺少关系，社区可能不准确。确保在创建 atoms 时也创建了合理的关系。

**Q: GraphRAG 的结果中 score 是什么含义？**

A: score 是 0-10 的综合评分，综合了语义相似度（30%）、图距离（25%）、类型重要性（20%）、时间新近度（15%）和关系链质量（10%）。分数越高表示与查询越相关。
